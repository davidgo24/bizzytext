...
from app.utils.messaging import notify_bizzy_about_new_web_booking
from app.services.conversation_state import ConversationStateManager
from fastapi.templating import Jinja2Templates
from app.utils.messaging import format_dt_human
...
async def confirm_booking(token: str, request: Request, session: Session = Depends(get_session)):
    ...
    slot_date = datetime.strptime(day_str.strip(), "%A %b %d").date()
    ...
    return RedirectResponse(url=f"/book/confirmation/{token}?name={client_name}&slot={format_dt_human(slot_dt)}", status_code=303)
